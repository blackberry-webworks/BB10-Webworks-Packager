...
function prepare(session) {
    ...
    webworksJsFiles = packagerUtils.listFiles(session.sourceDir, function (file) {
...
function copyWebworks(session) {
    ...
    if (srcFiles.length === 1) {
        ...
        fs.renameSync(path.join(dest, path.basename(srcFiles[0])), path.join(dest, "webworks.js"));
    } else {
