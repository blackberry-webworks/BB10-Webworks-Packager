...
var jWorkflow = require("jWorkflow"),
    ...
    packagerMinify = require("./packager-minify"),
...
function minifyContentTask(session) {
    return function (callback) {
        packagerMinify.minify(session, function () {
            callback();
        });
    };
}

function nativePackagerTask(session, target, config) {
    return function (callback) {
...
function buildTarget(previous, baton) {
    ...
    async.series([minifyContentTask(this.session),
                 nativePackagerTask(this.session, target, config)
    ], function (err, results) {
        baton.pass(results[1]);
    });
