...
function processSplashScreenIconSrc(data, widgetConfig, key) {
    var dataKey = (key === "splash" ? "rim:splash" : key);
    ...
    if (data[dataKey]) {
        ...
        if (!(data[dataKey] instanceof Array)) {
            if (data[dataKey]["@"]) {
                widgetConfig[key].push(data[dataKey]["@"].src);
            }
        } else {
            data[dataKey].forEach(function (obj) {
                if (obj["@"]) {
                    widgetConfig[key].push(data[dataKey]["@"].src);
                }
            });
...
function processSplashScreenData(data, widgetConfig) {
    processSplashScreenIconSrc(data, widgetConfig, "splash");
}
    ...
    if (widgetConfig[key]) {
        ...
        if (image) {
            Object.getOwnPropertyNames(image).forEach(function (lang) {
                if (lang === "default") {
                    msg = localize.translate("EXCEPTION_INVALID_IMAGE");
                } else {
                    msg = localize.translate("EXCEPTION_INVALID_TEXT");
                }

                if (image[lang] && image[lang] instanceof Array) {
                    image[lang].forEach(function (file) {
                        // if everything is specified correctly, file should be a string
                        // if it is an object and contains a property named "text", for sure
                        // the <text> element is either empty or does not specify language properly 
                        if (file.text) {
                            throw localize.translate("EXCEPTION_INVALID_TEXT");
                        } else {
                            check(file, msg)
                                .notNull()
                                .notEmpty();
...
//    validateSplashScreensIcon(widgetConfig, "icon");
    ...
    validateSplashScreensIcon(widgetConfig, "splash");
