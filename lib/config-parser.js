...
function processWidgetData(data, widgetConfig, session, extManager) {
    ...
    if (data["@"]) {
        ...
        widgetConfig.defaultlocale = data["@"].defaultlocale;
...
function collectAndCombineLocales(locales, localeElements) {
    var locale;
...
function processPackageLocale(widgetConfig) {
    var locales = [];

    // Push defaultlocale to be the first element in locales if specified
    if (widgetConfig.defaultlocale) {
        locales.push(widgetConfig.defaultlocale);
    }

    // collect and combine locale from "name" and "description"
    collectAndCombineLocales(locales, widgetConfig.name);
    collectAndCombineLocales(locales, widgetConfig.description);

    // If no locale is collected from "name" and "description", use the default value "en"
    if (locales.length === 0) {
        locales.push("en");
    }

    widgetConfig.packageLocale = locales.join();
}
...
function processResult(data, session, extManager) {
    ...
    processPackageLocale(widgetConfig); // It has to be processed after processWidgetData and processNameAndDescription
