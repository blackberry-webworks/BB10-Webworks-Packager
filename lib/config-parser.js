...
function createAccessListObj(featuresArray, uri, allowSubDomain) {
    ...
    if (featuresArray) {
        featuresArray.forEach(function (feature) {
			//To assert that the feature has the proper Id which is required as per the spec
            if (!feature["@"])
                throw localize.translate("EXCEPTION_INVALID_FEATURE_ID"); 
            
            check(feature["@"].id, localize.translate("EXCEPTION_INVALID_FEATURE_ID")).notEmpty();
            attribs = feature["@"];
