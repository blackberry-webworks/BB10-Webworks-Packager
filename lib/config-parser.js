...
function processSplashScreenIconImageData(data, widgetConfig, key) {
    if (data["rim:" + key] && data["rim:" + key]["image"]) {
        var rawImages = data["rim:" + key]["image"];

        widgetConfig[key] = {};
        widgetConfig[key]["image"] = {};
        ...
        rawImages.forEach(function (img) {
            ...
            if (img.text && img.text["@"] && img.text["@"]["xml:lang"]) {
                ...
                //if (typeof img.text["#"] === "string") {
                    //file = sanitize(img.text["#"]).trim();
                    file = trim(img.text["#"]);
                //} else {
                    // text element is specified and contain xml:lang attribute but is empty or contain nested element
                  //  throw localize.translate("EXCEPTION_INVALID_TEXT_EMPTY_OR_CONTAIN_NESTED_ELEM");
                //}
            } else {
                //if (typeof img === "string") {
                    //file = sanitize(img).trim();
                    file = trim(img);
                //} else {
                   // if (img.text) {
                        // text element is specified but xml:lang attribute is missing or empty
                     //   throw localize.translate("EXCEPTION_INVALID_TEXT_MISSING_LANG");
                    //} else {
                        // image element is empty or contain nested element that is not text
                      //  throw localize.translate("EXCEPTION_INVALID_IMAGE");
                   // }
                //}
            }

            if (!widgetConfig[key]["image"][lang]) {
                widgetConfig[key]["image"][lang] = [];
            }

            widgetConfig[key]["image"][lang].push(file);
        });
    } else {
        // this lets the validation logic knows that the element is specified but empty
        if (data["rim:" + key] && !data["rim:" + key]["image"]) {
            widgetConfig[key] = {};
        }
    }
...
function processIconData(data, widgetConfig) {
    ...
    // icon: {
    //    image: {
    //        "default": ["icon-86.png", "icon-90.png"],
    //        "fr": ["icon-90_fr.png"]
    //    }
    // }
    //
    processSplashScreenIconImageData(data, widgetConfig, "icon");
}
...
function processSplashScreenData(data, widgetConfig) {
    ...
    // }
    //
    processSplashScreenIconImageData(data, widgetConfig, "splashScreens");
}

function validateSplashScreensIcon(widgetConfig, key) {
    console.log("validateSplashScreensIcon");
    console.log(widgetConfig[key]);
    if (widgetConfig[key]) {
        var image = widgetConfig[key].image;

        if (image) {
            Object.getOwnPropertyNames(image).forEach(function (lang) {
                if (image[lang] && image[lang] instanceof Array) {
                    image[lang].forEach(function (file) {
                        if (typeof file === "string") {
                            check(file, localize.translate("EXCEPTION_INVALID_IMAGE"))
                                .notNull()
                                .notEmpty();
            ...
            if (key === "splashScreens") {
                throw localize.translate("EXCEPTION_EMPTY_SPLASH_SCREEN"); 
            } else {
                throw localize.translate("EXCEPTION_EMPTY_ICON");
            }
        }
    }
}
...
function validateConfig(widgetConfig) {
    ...
    validateSplashScreensIcon(widgetConfig, "splashScreens");
