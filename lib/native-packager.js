...
function generateTabletXMLFile(session, config) {
    ...
    if (config["splashScreens"]) {
        if (config["splashScreens"]["image"]) {
            xmlObject["splashScreens"] = {};
            xmlObject["splashScreens"]["image"] = [];

            Object.getOwnPropertyNames(config["splashScreens"]["image"]).forEach(function (lang) {
                if (lang === "default") {
                    config["splashScreens"]["image"][lang].forEach(function (img) {
                        xmlObject["splashScreens"]["image"].push({
                            _value: img
                        });
                    });
                } else {
