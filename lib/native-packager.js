...
/*
function generateSplashScreenIconData(config, xmlObject, key) {
    if (config[key] && config[key]["image"]) {
        xmlObject[key] = {};
        xmlObject[key]["image"] = [];

        Object.getOwnPropertyNames(config[key]["image"]).forEach(function (lang) {
            if (lang === "default") {
                config[key]["image"][lang].forEach(function (img) {
...
//    generateSplashScreenIconData(config, xmlObject, "splashScreens");
//    generateSplashScreenIconData(config, xmlObject, "icon");
